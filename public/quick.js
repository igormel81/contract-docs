import { sourceLabel, documentText, compactPassport, severityLabels, coverageLabels } from './document-ui.js';
...
export function textReport(packet, profileName) {
  ...
  for(const finding of result.findings)lines.push(`${finding.rule}: ${finding.title} (${finding.severity})`,finding.description,...(finding.proposal?[`Предлагаемая формулировка: ${finding.proposal}`]:[]),...refs(finding.sources),'');
...
export class QuickUI {
  ...
  rightView(){
    ...
    if(!p||p.status==='draft')return `<div class="flow analysis-result"><div class="empty"><h2>Одна проверка, без архива</h2><p>Загрузите договор с приложениями. Аналитик сформирует паспорт и замечания, ревьюер проверит выводы и предложенные правки по исходникам.</p><p>Заказчик, карточка договора и редакции не создаются.</p></div></div>`;
    return `<div class="flow analysis-result" data-result-key="${esc(p.id)}"><h2>Анализ и рекомендации</h2><h2 role="status">${esc(statuses[p.status])}</h2><div class="step-line"><div class="step ${p.primary_result?'done':p.status==='primary'?'running':''}">Аналитик<br>${p.primary_result?'Результат получен':'Ожидается'}</div><div class="step ${p.review_result?'done':p.status==='review'?'running':''}">Ревьюер<br>${p.review_result?'Проверено':'Не завершено'}</div></div>${p.error?`<p class="error" role="alert">${esc(p.error)}</p>`:''}${p.status==='error'?btn(p.primary_result?'Повторить ревью':'Повторить анализ','quick-run','','primary',!this.getBoot().codex.connected):''}${result?`${!p.review_result?'<p class="warning">Это первичный результат. Ревью ещё не завершено.</p>':''}<p>${esc(result.summary)}</p>${btn(p.review_result?'Скачать отчёт (.txt)':'Скачать первичный отчёт','quick-export','','primary')}${btn('Текст для менеджера','quick-summary','','compact-action')}${this.summary?`<section class="flow summary-panel"><div class="row between"><h3>Текст для менеджера</h3>${btn('Закрыть','quick-summary-close','','quiet compact-action')}</div><p class="muted">Собран из полученного результата без обращения к модели. В тексте указано, что результат нигде не сохранён.</p><div class="row">${btn('Скопировать','quick-summary-copy','','primary compact-action')}</div><label class="sr-only" for="summary-text">Текст замечаний</label><textarea id="summary-text" readonly rows="12">${esc(this.summary)}</textarea></section>`:''}<div class="warning"><p>Риски здесь не сохраняются: реестр, редакции и история ведутся только в хранилище.</p><div class="row">${btn('Вести этот договор постоянно','from-quick','','compact-action')}</div></div>${result.findings.map(f=>`<article class="finding"><small>${esc(f.rule)} · ${esc(severityLabels[f.severity])} критичность</small><h3>${esc(f.title)}</h3><p>${esc(f.description)}</p>${f.proposal?`<h3>Предлагаемая формулировка</h3><p class="source-block">${esc(f.proposal)}</p>`:''}<div>${this.refs(f.sources)}</div></article>`).join('')||'<p>Замечания не сформированы. Это не подтверждение отсутствия рисков.</p>'}<h3>Ограничения</h3>${result.limitations.map(l=>`<p class="muted">${esc(l)}</p>`).join('')}<details><summary>Покрытие правил</summary>${result.coverage.map(c=>`<p>${esc(c.rule)}: ${esc(coverageLabels[c.status]||c.status)}. ${esc(c.note)}</p>`).join('')}</details>`:'<p class="muted">Можно оставить эту страницу открытой. Результат появится автоматически, после обновления страницы текущую проверку можно продолжить.</p>'}</div>`;
  }
  view(){
