import { qualificationView, sourceLabel, documentText, compactPassport, severityLabels, coverageLabels, legalReferences, legalSnapshotLabel } from './document-ui.js';
...
export function textReport(packet, profileName) {
  ...
  lines.push('ПОКРЫТИЕ КРИТЕРИЕВ',...result.coverage.map(c=>`${c.rule}: ${c.status}. ${c.note}`),'','ОГРАНИЧЕНИЯ',...result.limitations,'Результат требует проверки сотрудником. Актуальность нормативных оснований указана в ограничениях проверки. OCR не подключён.');
...
export class QuickUI {
  ...
  // Executor-dependent disclosure: a one-off packet may leave the perimeter
  // (Codex, cloud vendor) or stay inside it (local model). Never claim one
  // while the installation runs the other.
  executorCapabilities(){return this.getBoot().codex?.capabilities||{provider:'codex',label:'Общий Codex',external:true,offline:'Общий Codex не подключён.',recovery:'Владелец приложения должен выполнить вход в настройках.'};}
  executorOffline(){const c=this.executorCapabilities();return `${c.offline} ${c.recovery}`;}
  executorTransfer(){const c=this.executorCapabilities();return c.provider==='codex'?'Текст передаётся в Codex.':c.external?`Текст передаётся облачному провайдеру (${c.provider}).`:'Текст обрабатывается моделью внутри контура установки и не передаётся наружу.';}
  executorProcessing(){const c=this.executorCapabilities();return c.provider==='codex'?'Режим не меняет условия обработки данных OpenAI.':c.external?'Режим не меняет условия обработки данных выбранного провайдера.':'Режим не меняет правила обработки данных внутри контура установки.';}
  ...
  rightView(){
    ...
    const statuses={draft:'Готов к загрузке',queued:'В общей очереди',qualification:'Определение типа договора',contract_risks:'Договорные риски',legal_modules:'Нормативная проверка',primary:'Первичный анализ',review:'Независимое ревью',complete:'Ревью завершено',error:'Ошибка этапа',cancelled:'Проверка отменена'};
  ...
  view(){
    ...
    return `<header class="context"><div><h1>Разовая проверка</h1></div><div class="row">${!p||p.status==='draft'?btn('Проверить пакет','quick-run','','primary',this.batch||!p?.files.length||p.files.some(f=>f.status!=='ready')||this.rows.some(r=>r.status==='error')||!this.getBoot().codex.connected):''}${p?this.controlMenu('quick-package','Действия с пакетом',btn('Удалить пакет','quick-discard','','danger menu-action'),{iconOnly:true}):''}</div></header><div class="quick-privacy"><p>${esc(this.executorTransfer())} Пакет и результат удаляются ${p?'до '+esc(this.date(p.expires)):'не позднее чем через час'}.</p><details><summary>Обработка и срок хранения</summary><p>Оригиналы удаляются после чтения. Перезапуск сервера завершает временную сессию. ${esc(this.executorProcessing())} Скачайте отчёт до удаления.</p></details>${this.error?`<p class="error" role="alert">${esc(this.error)}</p>`:''}</div><div class="panels ${showInspector?'has-inspector':'single-panel'} ${this.inspectorWide?'wide-inspector':''}"><section class="panel">${this.sectionTabs(true)}<div class="content">${this.tab==='analysis'?this.rightView():this.leftView()}</div></section><aside class="panel inspector ${showInspector?'':'hidden'}" aria-label="${this.tab==='analysis'?'Источник замечания':'Анализ и рекомендации'}"><div class="tabs row between"><span>${this.tab==='analysis'?'Источник замечания':'Анализ и рекомендации'}</span><div class="row">${btn(this.inspectorWide?'Уже':'Шире','quick-source-width','','quiet compact-action inspector-size')}${btn('Закрыть','quick-source-close','','quiet compact-action')}</div></div><div class="content">${this.tab==='analysis'?this.leftView('source'):hasResult?this.rightView():''}</div></aside></div>`;
  }
  async upload(files){
