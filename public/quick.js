...
export class QuickUI {
  ...
  view(){
    ...
    return `<header class="context"><div><h1>Разовая проверка</h1></div><div class="row">${!p||p.status==='draft'?btn('Проверить пакет','quick-run','','primary',this.batch||!p?.files.length||p.files.some(f=>f.status!=='ready')||this.rows.some(r=>r.status==='error')||!this.getBoot().codex.connected):''}${p?this.controlMenu('quick-package','Действия с пакетом',btn('Удалить пакет','quick-discard','','danger menu-action'),{iconOnly:true}):''}</div></header><div class="quick-privacy"><p>Текст передаётся в Codex. Пакет и результат удаляются ${p?'до '+esc(this.date(p.expires)):'не позднее чем через час'}.</p><details><summary>Обработка и срок хранения</summary><p>Оригиналы удаляются после чтения. Перезапуск сервера завершает временную сессию. Режим не меняет условия обработки данных OpenAI. Скачайте отчёт до удаления.</p></details>${this.error?`<p class="error" role="alert">${esc(this.error)}</p>`:''}</div><div class="panels ${showInspector?'has-inspector':'single-panel'} ${this.inspectorWide?'wide-inspector':''}"><section class="panel">${this.sectionTabs(true)}<div class="content">${this.tab==='analysis'?this.rightView():this.leftView()}</div></section><aside class="panel inspector ${showInspector?'':'hidden'}" aria-label="${this.tab==='analysis'?'Источник замечания':'Анализ и рекомендации'}"><div class="tabs row between"><span>${this.tab==='analysis'?'Источник замечания':'Анализ и рекомендации'}</span><div class="row">${btn(this.inspectorWide?'Уже':'Шире','quick-source-width','','quiet compact-action inspector-size')}${btn('Закрыть','quick-source-close','','quiet compact-action')}</div></div><div class="content">${this.tab==='analysis'?this.leftView('source'):hasResult?this.rightView():''}</div></aside></div>`;
  }
  async upload(files){
