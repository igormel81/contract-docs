...
export class OrganizationUI {
  ...
  form(){const {esc,btn}=this,d=this.data||{};
    // The INN lookup is a Codex-only capability: a local or cloud executor has
    // no internet search, and the server refuses the request outright.
    const canLookup=this.getBoot().codex?.capabilities?.organizationLookup!==false;
