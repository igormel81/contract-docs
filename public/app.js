...
function riskView() {
  ...
  return `<div class="flow">${pending.length?`<section class="flow"><div class="row between"><h3>Кандидаты из анализа${runRevision?' v'+runRevision.number:''} · ${pending.length}</h3></div><p class="muted">Замечания анализа не попадают в реестр автоматически. Подтвердите или отклоните каждое.</p>${pending.map(x=>`<article class="risk-card"><div class="row">${badge(labels[x.finding.severity],x.finding.severity)}${badge(x.finding.rule)}</div><h3>${esc(x.finding.title)}</h3><p class="muted">${esc(x.finding.description.slice(0,160))}</p><div class="row">${btn('В реестр рисков','finding-risk',x.index,'compact-action')}${btn('Отклонить','dismiss-candidate',x.index,'quiet compact-action')}</div></article>`).join('')}<hr class="separator"></section>`:''}<div class="row between"><h2>Реестр рисков</h2>${btn('+ Риск','new-risk','','compact-action')}</div><p class="muted">Постоянные записи по договору. Новая редакция не закрывает их автоматически.</p>${c.risks.map(r=>`<article class="risk-card"><div class="row">${badge(labels[r.severity],r.severity)}${badge(r.status)}</div><h3>${esc(r.title)}</h3><p class="muted">${esc(r.owner)}</p>${r.events.some(e=>e.kind==='incident')?badge('Есть события','medium'):''}${btn('Открыть','risk',r.id,'quiet compact-action')}</article>`).join('')||`<p>Риски пока не зарегистрированы. ${pending.length?'Выше — кандидаты последнего анализа.':'Добавьте вручную или из замечания анализа.'}</p>`}${dismissed.length?`<details><summary>Отклонённые кандидаты · ${dismissed.length}</summary>${dismissed.map(d=>`<article class="history-row"><small>${date(d.created)} · ${esc(d.rule)}</small><strong>${esc(d.title)}</strong><p class="muted">${esc(d.reason)}</p>${btn('Вернуть в кандидаты','restore-candidate',d.key,'quiet compact-action')}</article>`).join('')}</details>`:''}</div>`;
}
