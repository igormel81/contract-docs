...
function sidebar() {
  if(state.center==='quick')return `<aside class="sidebar">${btn('Разовая проверка','quick-open','','primary')}${btn('К хранилищу договоров','stored','','quiet')}<p class="muted">Загрузите пакет и получите рекомендации. Заказчик, карточка и редакции не нужны.</p><div class="sidebar-foot">Один временный пакет на пользователя.</div></aside>`;
...
function riskView() {
  ...
  return `<div class="flow">${pending.length?`<section class="flow"><div class="row between"><h3>Кандидаты из анализа${runRevision?' v'+runRevision.number:''} · ${pending.length}</h3></div><p class="muted">Замечания анализа не попадают в реестр автоматически. Подтвердите или отклоните каждое.</p>${pending.map(x=>`<article class="risk-card"><div class="row">${badge(labels[x.finding.severity],x.finding.severity)}${badge(x.finding.rule)}</div><h3>${esc(x.finding.title)}</h3><p>${esc(x.finding.description)}</p>${sources(x.finding.sources)}<div class="row">${btn('В реестр рисков','finding-risk',x.index,'compact-action')}${btn('Отклонить','dismiss-candidate',x.index,'quiet compact-action')}</div></article>`).join('')}<hr class="separator"></section>`:''}<div class="row between"><h2>Реестр рисков</h2>${btn('+ Риск','new-risk','','compact-action')}</div><p class="muted">Постоянные записи по договору. Новая редакция не закрывает их автоматически.</p>${c.risks.map(r=>`<article class="risk-card"><div class="row">${badge(labels[r.severity],r.severity)}${badge(r.status)}</div><h3>${esc(r.title)}</h3><p>${esc(r.detail)}</p><div>${r.sources?.length?r.sources.map((s,i)=>btn(esc('v'+s.revisionNumber+' · '+sourceLabel(s)),'risk-source',JSON.stringify([r.id,i]),'source-link')).join(''):'<small class="muted">Пункт не указан: риск заведён вручную или основание не сохранилось.</small>'}</div><div class="row"><small class="muted">Ответственный: ${esc(r.owner)}</small>${r.events.some(e=>e.kind==='incident')?badge('Есть события','medium'):''}${btn('Открыть','risk',r.id,'quiet compact-action')}</div></article>`).join('')||`<p>Риски пока не зарегистрированы. ${pending.length?'Выше — кандидаты последнего анализа.':'Добавьте вручную или из замечания анализа.'}</p>`}${dismissed.length?`<details><summary>Отклонённые кандидаты · ${dismissed.length}</summary>${dismissed.map(d=>`<article class="history-row"><small>${date(d.created)} · ${esc(d.rule)}</small><strong>${esc(d.title)}</strong><p class="muted">${esc(d.reason)}</p>${btn('Вернуть в кандидаты','restore-candidate',d.key,'quiet compact-action')}</article>`).join('')}</details>`:''}</div>`;
}
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='tab'){if(value==='document'&&state.center!=='analysis'){state.sourceDocuments=null;state.source=null;}state.center=value;state.form=null;}
    ...
    if(action==='source'){state.source=JSON.parse(value);state.sourceDocuments=state.source.analysisId?(await api('/analyses/'+state.source.analysisId+'/documents')).map(f=>({...f,extraction:f})):null;state.form=null;}
    ...
    if(action==='risk-source'){const [riskId,index]=JSON.parse(value),ref=state.contract.risks.find(r=>r.id===riskId).sources[index];state.source={...ref,riskId};state.sourceDocuments=[{id:ref.fileId,name:ref.fileName,extraction:{blocks:[ref.block],warnings:[]}}];state.form=null;}
