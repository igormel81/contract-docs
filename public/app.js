...
function centerView() {
  ...
  if(state.center==='rules')return `<div class="flow"><h2>Критерии проверки</h2>${legalCatalogue()}<p class="muted">Рекомендательный разбор для подготовки к переговорам, а не заключение о соответствии: правила формулируют вопросы и предложения, решение принимает сотрудник. Аналитик ищет полноту, ревьюер проверяет по оригиналам. Обе компании находятся в Москве. У каждого критерия своя версия: при её изменении прежние анализы не переписываются, а помечаются в истории.</p>${state.boot.rules.map(r=>`<section class="details-field"><div class="row">${badge(r.id)}${badge('версия '+r.version)}${r.coverage===false?badge('вне покрытия','medium'):''}</div><h3>${esc(r.title)}</h3><p>${esc(r.instruction)}</p>${r.avoid?`<p class="muted"><strong>Не считать замечанием:</strong> ${esc(r.avoid)}</p>`:''}</section>`).join('')}</div>`;
...
function render() {
  ...
  const c=state.contract,showInspector=Boolean(c&&!state.form&&state.inspectorOpen&&!['settings','rules'].includes(state.center));
  $('#app').innerHTML=`<header class="topbar"><div class="brand"><span class="mark" aria-hidden="true">Д</span>Договоры и риски</div><div class="row"><a class="installation-link" href="/docs/local-installation/" target="_blank" rel="noopener">Локальная установка</a>${btn('Настройки','settings','','quiet')}<small>${esc(state.user.login)}</small>${btn('Выйти','logout','','quiet')}</div></header><div class="workspace">${sidebar()}<main id="main" class="main ${state.center==='quick'?'quick-main':''}">${c?context():`<header class="context"><h1>${state.center==='settings'?'Настройки':'Договоры и шаблоны'}</h1></header>`}<div class="panels ${showInspector?'has-inspector':'single-panel'} ${state.inspectorWide?'wide-inspector':''}"><section class="panel"><nav class="tabs" aria-label="Разделы договора">${[['passport','Паспорт'],['analysis','Анализ'],['document','Документ'],['set','Комплект'],['risks','Риски'+(c&&candidates().length?' · кандидаты '+candidates().length:'')],['history','История']].map(([v,t])=>btn(t,'tab',v,state.center===v?'active':'',!c)).join('')}</nav><div class="content">${centerView()}</div></section><aside class="panel inspector ${showInspector?'':'hidden'}" aria-label="${c?esc(companionTitle()):'Источник'}"><div class="tabs row between"><span>${c?esc(companionTitle()):'Источник'}</span><div class="row">${btn(state.inspectorWide?'Уже':'Шире','source-width','','quiet compact-action inspector-size')}${btn('Закрыть','source-close','','quiet compact-action')}</div></div><div class="content">${c?companionView():'<div class="empty"><h3>От условий к решениям</h3><p>Здесь появятся замечания, предложенные формулировки и риски выбранного договора.</p></div>'}</div></aside></div></main></div><footer class="footer">Пилот 0.2.2 · оригиналы и история хранятся на сервере · AI-выводы требуют проверки сотрудником</footer>`;
