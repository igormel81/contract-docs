...
// Where document text is processed is a decision of this installation, and the
// login page states it before anyone signs in. /docs/health is public and
// carries only the executor kind, never an endpoint, key or model revision.
async function refreshService() { try { state.service=await (await fetch('/docs/health')).json(); } catch { /* Keep the cautious default below. */ } }
function serviceDisclosure() {
  const provider=state.service?.provider;
  if(provider==='local')return 'Текст договора обрабатывается моделью внутри контура установки и не передаётся наружу.';
  if(provider&&provider!=='codex')return `Текст договора передаётся облачному провайдеру (${provider}), выбранному администратором установки.`;
  return 'В текущей версии текст передаётся в Codex для анализа.';
}
...
// Codex is one executor among several. The settings screen must describe the
// one this installation actually runs, and must not offer a ChatGPT login that
// a local or cloud runner refuses (LOC-03).
const executor = () => state.boot?.codex?.capabilities
  || { provider:'codex', label:'Общий Codex', organizationLookup:true, external:true,
       offline:'Общий Codex не подключён.', recovery:'Обратитесь к владельцу приложения.' };
function settingsTail() {
  return `<hr class="separator"><h3>Ограничения пилота</h3><p class="muted">Нормативные основания и их актуальность показаны в критериях проверки. OCR пока не подключён. Результаты AI требуют проверки сотрудником. Договоры и история пользователей изолированы, несмотря на общий исполнитель анализа.</p><p class="muted">Загрузка до 20 МБ на файл, 200 МБ на аккаунт. История хранится на сервере установки.</p><hr class="separator"><form class="form" data-form="password"><h3>Сменить пароль приложения</h3>${field('current','Текущий пароль','','password','required autocomplete="current-password"')}${field('password','Новый пароль','','password','required minlength="10" maxlength="256" autocomplete="new-password"')}<div class="error" role="alert"></div><button type="submit">Сменить пароль и выйти</button></form></div>`;
}
function settingsView() {
  const connection=state.boot.codex, caps=executor();
  if(caps.provider!=='codex'){
    const pin=connection.inference||{};
    const facts=[`Модель: ${connection.model||pin.model||'—'}`,pin.modelRevision?`ревизия весов: ${pin.modelRevision}`:'',pin.contextWindow?`контекст: ${pin.contextWindow} токенов`:''].filter(Boolean).join(' · ');
    return `<div class="settings flow"><h2>Исполнитель анализа</h2><p>${esc(caps.label)} задан администратором установки при запуске службы. Вход в ChatGPT не используется; переключение исполнителя на работающем сервисе не поддерживается.</p><div class="${connection.connected?'info':'warning'}" role="status">${connection.connected?'Исполнитель отвечает. Доступность модели проверяется при запуске анализа.':esc(`${caps.offline} ${caps.recovery}`)}</div>${connection.error?`<div class="error" role="alert">${esc(connection.error)}</div>`:''}<p class="muted">${esc(facts)}</p><p class="muted">${caps.external?'Текст договора передаётся внешнему провайдеру; условия обработки данных определяет выбранный вендор.':'Текст договора не покидает контур установки: исполнителю разрешены только внутренние адреса сети.'}</p>${settingsTail()}`;
  }
  let control='';
...
function analysisView() {
  ...
  if(!a)return `<div class="flow analysis-result"><div class="empty"><h3>Проверка в два этапа</h3><p>Аналитик выявляет условия и риски. Ревьюер сверяет выводы и предложения с исходниками.</p>${state.boot.codex.connected?'':btn(executor().provider!=='codex'?'Состояние исполнителя':state.boot.codex.canManage?'Подключить общий Codex':'Статус общего Codex','settings','','primary')}</div></div>`;
