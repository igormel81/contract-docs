...
async function api(path, data, method='POST') {
  ...
  const payload=await response.json(); if(!response.ok){if(response.status===401&&state.user){state.user=null;render();} throw new Error(payload.error||'Не удалось выполнить запрос.');} return payload;
}
let noticeTimer;
...
async function openContract(id) { state.contractId=id;state.revisionId=null;state.runId=null;state.source=null;state.risk=null;state.form=null;await refreshContract();state.center=state.contract.revisions.length?'passport':'upload';history.replaceState(null,'','#'+id);render(); }
function sources(items=[]) { return items.map(s=>btn('К источнику', 'source', JSON.stringify(s),'source-link')).join(''); }
...
function sidebar() {
  ...
  return `<aside class="sidebar"><div class="nav">${btn('Договоры','kind','contract',state.kind==='contract'?'active':'')}${btn('Шаблоны','kind','template',state.kind==='template'?'active':'')}</div><label class="sr-only" for="search">Поиск по названию</label><input id="search" placeholder="Найти договор…" value="${esc(state.query)}"><label>Заказчик<select id="customer-filter"><option value="">Все заказчики</option>${state.boot.customers.map(c=>`<option value="${c.id}" ${state.customer===c.id?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><div class="row">${btn('+ '+(state.kind==='template'?'Шаблон':'Договор'),'new-contract','','grow')}${btn('+ Заказчик','new-customer','','compact-action')}</div><nav class="catalog" aria-label="Договоры">${list.length?list.map(c=>`<button type="button" data-action="open-contract" data-value="${c.id}" class="${state.contractId===c.id?'active':''}"><strong>${esc(c.title)}</strong><small>${esc(state.boot.profiles[c.contractor].name)}<br>${esc(c.stage)} · ${c.revision_count} ред.</small></button>`).join(''):'<p class="muted">Здесь появятся ваши документы.</p>'}</nav><div class="sidebar-foot">Москва · два профиля подрядчиков<br>${btn('Критерии проверки','rules','','quiet')}</div></aside>`;
}
function passportStrip() {
  const r=result(), a=analysis(); const same=a?.revision_id===state.revisionId;
  const value=key=>r?.passport.find(p=>p.key===key)?.value||'Не сформировано';
  return `<section class="passport-strip" aria-label="Краткий паспорт договора"><div class="row between"><span class="summary">${esc(same&&r?r.summary.length>220?r.summary.slice(0,220)+'…':r.summary:'Паспорт ещё не сформирован для выбранной редакции.')}</span>${btn('Подробнее','tab','passport','compact-action')}</div>${same&&r?`<div class="facts">${[['term','Сроки'],['price','Цена'],['payment','Оплата'],['location','Место']].map(([key,title])=>`<div class="fact"><span>${title}</span><strong>${esc(value(key).length>80?value(key).slice(0,80)+'…':value(key))}</strong></div>`).join('')}</div><div>${badge(a.review_result?'Ревью завершено · есть ограничения':'Черновик · без ревью',a.review_result?'':'medium')} ${badge('v'+(state.contract.revisions.find(x=>x.id===a.revision_id)?.number||'?'))}</div>`:'<small>После анализа здесь будут предмет, результат, сроки, цена, оплата и места работ.</small>'}</section>`;
}
function context() {
  const c=state.contract,customer=state.boot.customers.find(x=>x.id===c.customer_id);
  return `<header class="context"><div class="grow"><p>${esc(customer?.name||'Библиотека шаблонов')} / ${esc(state.boot.profiles[c.contractor].name)}</p><h1>${esc(c.title)}</h1></div><div class="row"><label class="sr-only" for="revision-select">Редакция</label><select id="revision-select"><option value="">Без редакции</option>${c.revisions.map(r=>`<option value="${r.id}" ${r.id===state.revisionId?'selected':''}>v${r.number}${c.effective_id===r.id?' · действует':''}</option>`).join('')}</select>${btn('Анализировать','analyze','','primary',!state.revisionId||c.analyses.some(a=>a.revision_id===state.revisionId&&['queued','primary','review'].includes(a.status)))}</div></header>`;
}
function uploadView() {
...
function documentView() {
  const files=state.contract.files; const file=files.find(f=>f.id===state.source?.fileId)||files.find(f=>revision()?.file_ids.includes(f.id))||files[0];
  ...
  return `<div class="flow"><div class="row"><label class="grow">Документ<select id="document-select">${files.map(f=>`<option value="${f.id}" ${f.id===file.id?'selected':''}>${esc(f.name)}</option>`).join('')}</select></label><a href="/docs/api/files/${file.id}/download" class="compact-action">Скачать оригинал</a></div>${state.source?.blockId?btn('Назад к паспорту','tab','passport','quiet'):''}<small>Извлечённый текст. Номера фрагментов — технические ссылки, не номера пунктов договора.</small>${file.extraction?.warnings?.length?`<div class="warning">${file.extraction.warnings.map(esc).join('<br>')}</div>`:''}<div class="document">${file.extraction?.blocks?.length?file.extraction.blocks.map(b=>`<section class="source-block ${state.source?.blockId===b.id?'highlight':''}" id="source-${b.id}"><small>${esc(b.id)}${b.page?' · страница '+b.page:''}</small>${esc(b.text)}</section>`).join(''):'<p class="warning">Текст пока недоступен. Оригинал сохранён.</p>'}</div></div>`;
}
function passportView() {
  ...
  return `<div class="flow"><div class="row between"><h2>Паспорт · v${revision()?.number}</h2>${badge(a.review_result?'С ревью':'Без ревью',a.review_result?'':'medium')}</div><p>${esc(r.summary)}</p>${r.passport.map(f=>`<section class="details-field"><div class="row between"><h3>${esc(f.title)}</h3>${badge(labels[f.status],f.status==='extracted'?'':'medium')}</div><p>${esc(f.value)}</p>${sources(f.sources)}</section>`).join('')}<div class="warning">${r.limitations.map(esc).join('<br>')}</div><p class="muted">Паспорт относится только к выбранному комплекту и не означает юридического согласования.</p></div>`;
}
...
function settingsView() {
  ...
  return `<div class="settings flow"><h2>Подключение Codex</h2><p>Два отдельных этапа через вашу авторизацию ChatGPT. API-ключи не используются. Подключение принадлежит только вашему аккаунту приложения.</p>${connection.connected?`<div class="info">Вход ChatGPT сохранён. Доступность анализа проверяется при запуске.</div>${btn('Отключить Codex','codex-logout','','danger')}`:`<div class="warning">Codex не подключён. До входа доступны документы, редакции и ручной реестр рисков.</div>${connection.login?.state==='pending'?`<div class="flow"><p>${connection.login.code?'Откройте страницу входа и введите одноразовый код:':'Получаем код входа…'}</p>${connection.login.code?`<h2 class="mono">${esc(connection.login.code)}</h2>`:''}${connection.login.url?`<a href="${esc(connection.login.url)}" target="_blank" rel="noopener noreferrer">Открыть официальный вход ChatGPT</a>`:''}<small>Код виден только вам. Не передавайте его другим людям.</small></div>`:btn('Войти через ChatGPT','codex-login','','primary')}${connection.login?.error?`<div class="error">${esc(connection.login.error)}</div>`:''}`}<hr class="separator"><h3>Ограничения пилота</h3><p class="muted">Нормативная база и OCR пока не подключены. Результаты AI требуют проверки сотрудником. Документы каждого пользователя изолированы; совместный доступ и роли добавим отдельно.</p><p class="muted">Загрузка до 20 МБ на файл, 200 МБ на аккаунт. История хранится на VPS.</p><hr class="separator"><form class="form" data-form="password"><h3>Сменить пароль приложения</h3>${field('current','Текущий пароль','','password','required autocomplete="current-password"')}${field('password','Новый пароль','','password','required minlength="10" maxlength="256" autocomplete="new-password"')}<div class="error" role="alert"></div><button type="submit">Сменить пароль и выйти</button></form></div>`;
}
function centerForm() {
  const f=state.form;
...
function analysisView() {
  ...
  if(!a)return `<div class="empty"><h3>Проверка в два этапа</h3><p>Аналитик выявляет условия и риски. Ревьюер сверяет выводы и предложения с исходниками.</p>${state.boot.codex.connected?'':btn('Подключить Codex','settings','','primary')}</div>`;
...
function riskView() {
  ...
  const f=state.form;
  if(f?.type==='risk')return formWrap('risk','Зафиксировать риск',field('title','Название',f.finding?.title||'','text','required maxlength="200"')+select('severity','Критичность',[['high','Высокая'],['medium','Средняя'],['low','Низкая']],f.finding?.severity||'medium')+field('owner','Ответственный',state.user.login,'text','required maxlength="100"')+area('detail','Причина, событие и последствия',f.finding?.description||'','required maxlength="5000"'),'Добавить в реестр');
  ...
  if(risk)return `<div class="flow">${btn('Все риски','all-risks','','quiet')}<div class="row">${badge(labels[risk.severity],risk.severity)}${badge(risk.status)}</div><h2>${esc(risk.title)}</h2><p>${esc(risk.detail)}</p><small>Ответственный: ${esc(risk.owner)} · ${date(risk.created)}</small><div class="row">${btn('Мера снижения','risk-event','mitigation')}${btn('Событие','risk-event','incident')}${btn('Статус','risk-status')}</div>${risk.events.map(e=>`<article class="history-row"><small>${date(e.created)}${e.due?' · дата / срок '+esc(e.due):''}</small><div class="row">${badge(e.kind==='incident'?'Событие':e.kind==='mitigation'?'Мера':e.kind==='decision'?'Решение':'Заметка')}${badge(labels[e.state]||e.state)}</div><p>${esc(e.text)}</p>${['mitigation','incident'].includes(e.kind)?btn('Проверить / обновить','event-status',e.id,'quiet compact-action'):''}</article>`).join('')||'<p class="muted">Мер и событий пока нет.</p>'}</div>`;
  return `<div class="flow"><div class="row between"><h2>Реестр рисков</h2>${btn('+ Риск','new-risk','','compact-action')}</div><p class="muted">Постоянные записи по договору. Новая редакция не закрывает их автоматически.</p>${c.risks.map(r=>`<article class="risk-card"><div class="row">${badge(labels[r.severity],r.severity)}${badge(r.status)}</div><h3>${esc(r.title)}</h3><p class="muted">${esc(r.owner)}</p>${r.events.some(e=>e.kind==='incident')?badge('Есть события','medium'):''}${btn('Открыть','risk',r.id,'quiet compact-action')}</article>`).join('')||'<p>Риски пока не зарегистрированы. Добавьте вручную или из замечания анализа.</p>'}</div>`;
}
...
function render() {
  ...
  $('#app').innerHTML=`<header class="topbar"><div class="brand"><span class="mark" aria-hidden="true">Д</span>Договоры и риски<span class="small-label">Кастис / Модеус</span></div><div class="row">${btn(state.boot.codex.connected?'Codex · вход сохранён':'Подключить Codex','settings','','quiet')}<small>${esc(state.user.login)}</small>${btn('Выйти','logout','','quiet')}</div></header><div class="workspace">${sidebar()}<main id="main" class="main">${c?context()+passportStrip():`<header class="context"><h1>${state.center==='settings'?'Настройки':'Договоры и шаблоны'}</h1></header><div></div>`}<div class="panels"><section class="panel"><nav class="tabs" aria-label="Работа с договором">${[['passport','Паспорт'],['document','Документ'],['upload','Загрузка'],['versions','Редакции']].map(([v,t])=>btn(t,'tab',v,state.center===v?'active':'',!c)).join('')}</nav><div class="content">${centerView()}</div></section><aside class="panel inspector"><nav class="tabs" aria-label="Результаты проверки">${[['analysis','Анализ'],['risks','Риски'],['history','История']].map(([v,t])=>btn(t,'right',v,state.right===v?'active':'',!c)).join('')}</nav><div class="content">${c?({analysis:analysisView,risks:riskView,history:historyView}[state.right])():'<div class="empty"><h3>От условий к решениям</h3><p>Здесь появятся замечания, предложенные формулировки и риски выбранного договора.</p></div>'}</div></aside></div></main></div><footer class="footer">Пилот 0.1 · оригиналы и история хранятся на сервере · AI-выводы требуют проверки сотрудником</footer>`;
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='logout'){await api('/logout',{});state.user=null;state.boot=null;state.contract=null;state.queues.clear();state.selections.clear();render();return;}
    if(action==='open-contract'){await openContract(value);return;}
    if(action==='kind'){state.kind=value;state.customer='';}
    if(action==='tab'){state.center=value;state.form=null;}
    ...
    if(action==='file'){state.source={fileId:value};state.center='document';state.form=null;}
    if(action==='source'){state.source=JSON.parse(value);state.center='document';state.form=null;}
    ...
    if(action==='codex-login'){state.boot.codex=await api('/codex/login',{});notice('Начат вход в Codex. Дождитесь одноразового кода.');}
    if(action==='codex-logout'){await api('/codex/logout',{});state.boot.codex=await api('/codex');notice('Codex отключён, активные анализы отменены.');}
    render();
...
document.addEventListener('submit',async event=>{
  ...
  try{
    ...
    if(type==='auth'){state.user=await api('/'+state.loginMode,data);await refreshBoot();state.contractId=null;state.contract=null;state.form=null;const fromHash=location.hash.slice(1);if(state.boot.contracts.some(c=>c.id===fromHash))await openContract(fromHash);else render();return;}
    ...
    if(type==='risk'){const value=await api('/contracts/'+state.contractId+'/risks',{...data,origin:state.form.analysis?state.form.analysis+':'+state.form.finding.id:''});state.risk=value.id;}
...
setInterval(async()=>{
  ...
  try{
    if(state.center==='settings'&&state.boot.codex.login?.state==='pending'){state.boot.codex=await api('/codex');if(!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName))render();}
...
try{state.user=await api('/me');await refreshBoot();const fromHash=location.hash.slice(1);if(state.boot.contracts.some(c=>c.id===fromHash))await openContract(fromHash);else render();}catch{render();}
