...
function riskView() {
  ...
  const f=state.form;
  if(f?.type==='risk')return formWrap('risk','Зафиксировать риск',field('title','Название',f.finding?.title||'','text','required maxlength="200"')+select('severity','Критичность',[['high','Высокая'],['medium','Средняя'],['low','Низкая']],f.finding?.severity||'medium')+field('owner','Ответственный',state.user.login,'text','required maxlength="100"')+`${f.finding?'<div>'+sources(f.finding.sources)+'</div>':select('source','Пункт и редакция (если риск связан с условием)',[['','Нет конкретного пункта'],...(revision()?.file_ids||[]).flatMap(id=>{const file=c.files.find(x=>x.id===id);return (file?.extraction?.blocks||[]).map(b=>[JSON.stringify({fileId:id,blockId:b.id,revisionId:state.revisionId}),file.name+': '+locationLabel(b)+' '+b.text.slice(0,90)]);})])}`+area('detail','Причина, событие и последствия',f.finding?.description||'','required maxlength="5000"'),'Добавить в реестр');
