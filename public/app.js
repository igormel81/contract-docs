...
const btn = (title, action, value='', cls='', disabled=false) => `<button type="button" data-action="${action}" data-value="${esc(value)}" class="${cls}" ${['tab','quick-tab','kind','open-contract'].includes(action)&&cls==='active'?'aria-current="page"':''} ${['finding-filter','quick-filter'].includes(action)?'aria-pressed="'+(cls.includes('active'))+'"':''} ${disabled?'disabled':''}>${title}</button>`;
...
function render() {
  if(!state.user){state.drafts.clear();quick.reset();organizations.reset();$('#app').innerHTML=loginView();return;}
  ...
  $('#app').innerHTML=`<header class="topbar"><div class="brand"><img class="mark" src="/docs/logo.svg" width="30" height="34" alt="" aria-hidden="true">Договоры и риски</div><div class="row"><a class="installation-link" href="/docs/local-installation/" target="_blank" rel="noopener">Локальная установка</a><a class="installation-link" href="https://github.com/igormel81/contract-docs" target="_blank" rel="noopener noreferrer" aria-label="Код на GitHub (новая вкладка)">Код на GitHub</a>${btn('Организации','organizations','','quiet')}${btn('Настройки','settings','','quiet')}<small>${esc(state.user.login)}</small>${btn('Выйти','logout','','quiet')}</div></header><div class="workspace">${sidebar()}<main id="main" class="main ${state.center==='quick'?'quick-main':''}">${c?context():`<header class="context"><h1>${state.center==='settings'?'Настройки':state.center==='organizations'?'Мои организации':'Договоры и шаблоны'}</h1></header>`}<div class="panels ${showInspector?'has-inspector':'single-panel'} ${state.inspectorWide?'wide-inspector':''}"><section class="panel"><nav class="tabs ${state.center==='organizations'?'hidden':''}" aria-label="Разделы договора">${[['passport','Паспорт'],['analysis','Анализ'],['document','Документ'],['set','Комплект'],['risks','Риски'+(c&&candidates().length?' · кандидаты '+candidates().length:'')],['history','История']].map(([v,t])=>btn(t,'tab',v,state.center===v?'active':'',!c)).join('')}</nav><div class="content">${centerView()}</div></section><aside class="panel inspector ${showInspector?'':'hidden'}" aria-label="${c?esc(companionTitle()):'Источник'}"><div class="tabs row between"><span>${c?esc(companionTitle()):'Источник'}</span><div class="row">${btn(state.inspectorWide?'Уже':'Шире','source-width','','quiet compact-action inspector-size')}${btn('Закрыть','source-close','','quiet compact-action')}</div></div><div class="content">${c?companionView():'<div class="empty"><h3>От условий к решениям</h3><p>Здесь появятся замечания, предложенные формулировки и риски выбранного договора.</p></div>'}</div></aside></div></main></div><footer class="footer">Пилот 0.3.2 · оригиналы и история хранятся на сервере · AI-выводы требуют проверки сотрудником</footer>`;
...
function syncSourceOverlay(){
  ...
  document.querySelectorAll('.topbar,.sidebar,.footer,#main>.context,#main>.quick-privacy,#main>.panels>section.panel').forEach(el=>{el.inert=modal;});
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='analyze'){const started=await api('/contracts/'+state.contractId+'/analyses',{revision_id:state.revisionId});state.source=null;state.sourceDocuments=null;state.runId=null;state.center='analysis';await refreshContract();notice(started.note?'Комплект в очереди. '+started.note:'Комплект поставлен в очередь анализа.');}
