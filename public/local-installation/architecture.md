...
> **Статус:** проект целевой архитектуры; в 0.3.3 реализованы изолированные основы local provider, opt-in OCR и импорта нормативных пакетов, но рабочий runner, интерфейс выбора и offline-развёртывание ещё не готовы.
> **Дата:** 2026-09-07. **Базовый срез:** модульная основа 0.3.3, commit `6004d92`; действующее приложение использует общий Codex.
...
| Модель | Активен `CodexRunner` в `server/codex.mjs`: два `codex exec`, общая авторизация приложения, инструменты отключены. Изолированный HTTP-provider есть как проверенный модуль, но к runner и UI не подключён |
