...
> **Статус:** проект целевой архитектуры. В 0.3.4 локальный и облачные исполнители подключены к рабочему runner за стартовой настройкой `DOCS_MODEL_PROVIDER`, интерфейс и отказы описывают активного исполнителя. Не готовы: переключение режима на работающем инстансе (LOC-03), offline-инсталлятор (LOC-08), приёмка на реальном vLLM/GPU и оценка качества. OCR и импорт нормативных пакетов остаются проверенными модулями без интерфейса.
> **Дата:** 2026-09-22. **Базовый срез:** поставка 0.3.4; публичный пилот работает на общем Codex, установка в организации выбирает исполнителя при запуске.
...
| Модель | Исполнитель фиксируется при старте (`modelProviderConfiguration` в `server/main.mjs`): `CodexRunner` — два `codex exec` с общей авторизацией приложения и отключёнными инструментами; `LocalRunner` — внутренний vLLM с закреплёнными весами, токенизатором и chat template и запретом внешних адресов; `CloudRunner` — вендорский API по ключу организации. Переключение на работающем инстансе не поддерживается, запасного выхода в облако нет |
...
Проверенные исходники: API (в архиве: server/main.mjs), исполнитель (в архиве: server/codex.mjs), схемы (в архиве: server/schema.mjs), разовый режим (в архиве: server/quick-checks.mjs), извлечение (в архиве: server/documents.mjs), структура документа (в архиве: server/extract.py), источники (в архиве: server/sources.mjs), данные (в архиве: server/db.mjs).

**Дополнение к выпуску 0.2.0:** параллельно подключён начальный корпус из шести норм ГК РФ, часть 2 (ст. 702, 706, 708, 709), с хешами текста, метаданными и неизменяемой копией `snapshot.legal`; замечания получают проверяемые `legalSources`. Корпус имеет статус `reference_only`, `currentAsOf=null`: текущая редакция по официальному источнику не подтверждена, полная правовая проверка не заявляется. Это начальная справочная интеграция; подписанный offline-import, подтверждённая актуальность и полнота корпуса остаются TO-BE. Точное состояние и ограничения: [Нормативная база](normative.md).
...
Уточнение владельца от 2026-09-05: **Codex остаётся режимом по умолчанию; локальная модель — опциональный второй режим той же установки.** Выбор делает администратор приложения, не каждый пользователь. Переключение допускается только вне активных/ожидающих заданий и операции входа. Оно не удаляет серверную авторизацию Codex; явный logout остаётся отдельным действием. Провайдер фиксируется для каждого запуска, его этапов и повторов; автоматического fallback нет. В local mode Codex не вызывается скрытно для поиска организации.

В исходниках сохраняется только реализация входа. Новая установка не получает сессию владельца проекта и требует собственной авторизации. Токены, auth.json, рабочие конфигурации, договоры, базы и резервные копии находятся вне Git и исходных архивов; примеры обезличены. Эти требования и проверки включены в LOC-01…04 плана (в архиве: plans/features/(sep-26)-local-installation.md). Выбор исполнителя подключён к приложению как стартовая настройка; интерфейс администратора и переключение без перезапуска остаются в LOC-03.
...
- План реализации полной локальной установки (в архиве: plans/features/(sep-26)-local-installation.md).
- [Пошаговое развёртывание текущей версии и план локального перехода](deployment.md).
- Текущее устройство, запуск и ограничения пилота (в архиве: README.md).
- Правила и инструкции двух этапов (в архиве: server/rules.mjs).
- Схемы и проверки доказательств (в архиве: server/schema.mjs).
- [Начальная нормативная база и границы проверки](normative.md).
- Оценка правил на модели (в архиве: scripts/rules-eval.mjs).
- Эксплуатационный журнал и конфигурация рабочего сервера не входят в публичную исходную поставку.

План локальной миграции создан и связан выше. Отдельные ADR и протокол аппаратных испытаний пока не созданы. Этот документ не означает, что перечисленные компоненты уже входят в deploy текущего сервиса.

## Update Log

- 2026-09-22: отражена поставка 0.3.4 — local provider и облачные вендоры подключены к рабочему runner, интерфейс и отказы следуют активному исполнителю. OCR и подписанные нормативные пакеты остаются модулями без интерфейса; промышленная локальная установка по-прежнему не заявляется.
- 2026-09-07: отражена поставка 0.3.3 — модульные основы local provider, OCR и подписанных нормативных пакетов реализованы, но не активированы; промышленная локальная установка не заявляется.
- 2026-09-05: закреплены два опционально выбираемых режима с Codex по умолчанию, сохранением серверной сессии и запретом поставки персональной авторизации в исходниках.
- 2026-09-05: добавлена связь с исполняемым планом локальной установки; старт модулей не означает готовность offline-поставки.
