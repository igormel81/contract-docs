...
test('account isolation, uploads, immutable revisions, risks, CSRF and session revocation',async t=>{
  ...
  for(const limitation of run.review_result.limitations)assert.ok(letter.text.includes(limitation),'The manager report preserves every limitation from the reviewed result');
