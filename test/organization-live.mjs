...
try{
  ...
  const result=await app.runner.organizationLookup('live-public-inn-smoke',randomUUID(),'7707083893',()=>true);
  ...
  console.log(JSON.stringify({status:'PASS',inn:result.inn,name:result.name,sourceCount:result.sources.length,sourceUrls:[...new Set(result.sources.map(s=>s.url))],note:result.note,disclaimer:'Live search result only, not independently verified registry extract; nothing saved in service'}));
