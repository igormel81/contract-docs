...
const corpusPath = new URL('./legal-data/contract-review-2026-09-22.json', import.meta.url);
const expectedSha256 = '4ba9828a53cd7875728bc4a766f3960ede37586017d7285353a421f59930df61';
...
LAW-01: сначала установи вид каждого существенного обязательства и выбери только применимые к нему активные модули. civil-general охватывает отдельные правила ответственности предпринимателя и уменьшения неустойки; civil-works — результат работ, сроки, цену, содействие и приемку; civil-services — действия или деятельность, оплату и односторонний отказ; software-rights — отчуждение, лицензию, сублицензию и ПО по заказу; personal-data — только фактическое поручение обработки, локализацию, безопасность и трансграничную передачу; commercial-secrets — условия установления режима коммерческой тайны. Для mixed квалифицируй каждый блок отдельно. Не достраивай нормы, исключения, отсылочные статьи и судебную практику по памяти.
Корпус не покрывает 44-ФЗ, 223-ФЗ, налоги, лицензируемую деятельность, СРО, защиту потребителей, трудовые отношения, отраслевые требования, иностранное право и санкции. По непокрытому вопросу допустимы только facts_or_documents_required или not_applicable и явное ограничение.
