...
function shell(title,content,toc=[]){return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${escapeHtml(title)} — Договоры и риски</title><link rel="stylesheet" href="../app.css"><link rel="stylesheet" href="publication.css"></head><body class="publication"><a class="skip" href="#document">К документу</a><header class="publication-header"><a href="/docs/">Договоры и риски</a><nav aria-label="Документация"><a href="./">Локальная установка</a><a href="architecture.html">Архитектура</a><a href="deployment.html">Инструкция</a></nav></header><div class="publication-layout">${toc.length?`<aside class="publication-toc"><details><summary>В этом документе</summary><nav aria-label="Оглавление">${toc.filter(h=>h.level===2).map(h=>`<a href="#${h.id}">${escapeHtml(h.text)}</a>`).join('')}</nav></details></aside>`:''}<main id="document" class="publication-content">${content}</main></div><footer class="publication-footer">Поставка 0.2.1 · 5 сентября 2026 · документ доступен без входа. Для PDF используйте печать браузера.</footer></body></html>`;}
...
export function buildBundle(){
  ...
  const stage=mkdtempSync(join(tmpdir(),'docs-source-')),name='contract-docs-0.2.1',folder=join(stage,name),entries=[];
  try {
    mkdirSync(folder);
    ...
    execFileSync('tar',['-czf',join(out,filename),'-C',stage,name],{env:{...process.env,COPYFILE_DISABLE:'1'}});
    ...
    writeFileSync(join(out,'manifest.json'),JSON.stringify({version:'0.2.1',sourceCommit:commit,archive:filename,bytes:archive.length,sha256:archiveHash,files:entries},null,2)+'\n');
