...
export function buildBundle(){
  ...
  const stage=mkdtempSync(join(tmpdir(),'docs-source-')),name='contract-docs-0.2.2',folder=join(stage,name),entries=[];
  try {
    mkdirSync(folder);
    ...
    writeFileSync(join(out,'manifest.json'),JSON.stringify({version:'0.2.2',sourceCommit:commit,archive:filename,bytes:archive.length,sha256:archiveHash,files:entries},null,2)+'\n');
