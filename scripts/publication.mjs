...
export function buildDocuments(){
  ...
  writeFileSync(join(out,'index.html'),shell('Локальная установка',`<p class="publication-meta">Архитектура и исходная поставка · 0.3.3</p><h1>Развернуть сервис<br>в организации</h1><p class="publication-lead">Устройство сервиса, инструкция для администратора и файлы проекта — в одном комплекте.</p><p class="publication-callout"><strong>Локальные модели пока не подключены к приложению.</strong> В архив включены проверенные модули локального провайдера, OCR и импорта подписанных нормативных пакетов; рабочий режим по умолчанию — общий Codex.</p><section class="publication-section"><h2>Документы</h2><a class="document-card" href="architecture.html"><span><strong>Архитектура локальной установки</strong><small>Компоненты, данные, два этапа анализа, модели, безопасность и оборудование</small></span><span aria-hidden="true">→</span></a><a class="document-card" href="deployment.html"><span><strong>Пошаговое развёртывание</strong><small>Требования, systemd, HTTPS, первый вход, проверки, резервные копии и откат</small></span><span aria-hidden="true">→</span></a><a class="document-card" href="normative.html"><span><strong>Нормативная база</strong><small>6 положений о подряде; актуальность редакции не подтверждена</small></span><span aria-hidden="true">→</span></a></section><section class="publication-section" id="downloads"><h2>Файлы проекта</h2><p>Исходники, тесты, документы и примеры конфигурации. Без договоров, базы данных, секретов и истории Git. Зависимости и веса моделей не включены.</p><div class="document-actions"><a class="download-primary" href="/docs/downloads/contract-docs-0.3.3.tar.gz" download>Скачать проект · TAR.GZ</a><a href="/docs/downloads/contract-docs-0.3.3.tar.gz.sha256" download>Контрольная сумма SHA-256</a></div><p><a href="/docs/downloads/manifest.json">Состав и версия поставки</a></p></section>`));
}
...
export function buildBundle(){
  ...
  const stage=mkdtempSync(join(tmpdir(),'docs-source-')),name='contract-docs-0.3.3',folder=join(stage,name),entries=[];
  try {
    mkdirSync(folder);
    ...
    add('README.md',Buffer.from('# Договоры и риски — исходная поставка 0.3.3\n\nНачните с [инструкции](references/local-deployment.md).\n\nЛокальный провайдер, OCR и импорт нормативных пакетов включены как неподключённые модули. Активный runner использует Codex. Зависимости, веса, пользовательские данные и секреты не включены.\n'));
    ...
    writeFileSync(join(out,'manifest.json'),JSON.stringify({version:'0.3.3',sourceCommit:commit,archive:filename,bytes:archive.length,sha256:archiveHash,files:entries},null,2)+'\n');
