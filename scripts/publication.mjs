...
// Version and release date follow package.json and the guide instead of three
// hand-copied literals that drifted apart across releases.
const VERSION=JSON.parse(readFileSync(join(root,'package.json'),'utf8')).version;
const RELEASE_DATE='22 сентября 2026';
...
// The .md beside each page is offered as a download, so its links have to work
// from that directory. Sibling documents become <slug>.md; repository paths,
// which do not exist next to the published page, become the archive path — the
// same resolution the HTML performs.
export function publishedMarkdown(markdown){
  return markdown.replace(/\[([^\]]+)\]\((?:<([^>]+)>|([^\s)]+))\)/g,(whole,label,angled,plain)=>{
    const url=angled||plain;
    if(/^(?:https?:\/\/|#|mailto:)/.test(url))return whole;
    const page=pages.find(x=>url.endsWith(x[1].split('/').at(-1)));
    if(page)return `[${label}](${page[0]}.md)`;
    return `${label} (в архиве: ${url.replace(/^\.\.\//,'')})`;
  });
}
...
function shell(title,content,toc=[]){return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${escapeHtml(title)} — Договоры и риски</title><link rel="icon" href="/docs/logo.svg" type="image/svg+xml"><link rel="stylesheet" href="../app.css"><link rel="stylesheet" href="publication.css"></head><body class="publication"><a class="skip" href="#document">К документу</a><header class="publication-header"><a class="brand" href="/docs/"><img class="mark" src="/docs/logo.svg" width="30" height="34" alt="" aria-hidden="true">Договоры и риски</a><nav aria-label="Документация"><a href="./">Локальная установка</a><a href="architecture.html">Архитектура</a><a href="deployment.html">Инструкция</a><a href="https://github.com/igormel81/contract-docs" target="_blank" rel="noopener noreferrer" aria-label="Код на GitHub (новая вкладка)">Код на GitHub</a></nav></header><div class="publication-layout">${toc.length?`<aside class="publication-toc"><details><summary>В этом документе</summary><nav aria-label="Оглавление">${toc.filter(h=>h.level===2).map(h=>`<a href="#${h.id}">${escapeHtml(h.text)}</a>`).join('')}</nav></details></aside>`:''}<main id="document" class="publication-content">${content}</main></div><footer class="publication-footer">Поставка ${VERSION} · ${RELEASE_DATE} · документ доступен без входа. Для PDF используйте печать браузера.</footer></body></html>`;}
export function buildDocuments(){
  const out=join(root,'public/local-installation');mkdirSync(out,{recursive:true});
  for(const [slug,path,label] of pages){const md=readFileSync(join(root,path),'utf8');const rendered=renderMarkdown(md);writeFileSync(join(out,`${slug}.md`),publishedMarkdown(md));writeFileSync(join(out,`${slug}.html`),shell(label,`<div class="document-actions"><a href="${slug}.md" download>Скачать документ · Markdown</a></div>`+rendered.html,rendered.toc));}
  writeFileSync(join(out,'index.html'),shell('Локальная установка',`<p class="publication-meta">Архитектура и исходная поставка · ${VERSION}</p><h1>Развернуть сервис<br>в организации</h1><p class="publication-lead">Устройство сервиса, инструкция для администратора и файлы проекта — в одном комплекте.</p><p class="publication-callout"><strong>Исполнитель анализа выбирается при установке.</strong> Общий Codex остаётся режимом по умолчанию; локальная модель во внутренней сети и облачные вендоры подключены и настраиваются переменной <code>DOCS_MODEL_PROVIDER</code>. Приёмка локального режима на реальном GPU-стенде ещё не выполнена. OCR и импорт подписанных нормативных пакетов остаются модулями без интерфейса.</p><section class="publication-section"><h2>Документы</h2><a class="document-card" href="architecture.html"><span><strong>Архитектура локальной установки</strong><small>Компоненты, данные, два этапа анализа, модели, безопасность и оборудование</small></span><span aria-hidden="true">→</span></a><a class="document-card" href="deployment.html"><span><strong>Пошаговое развёртывание</strong><small>Требования, systemd, HTTPS, первый вход, проверки, резервные копии и откат</small></span><span aria-hidden="true">→</span></a><a class="document-card" href="normative.html"><span><strong>Нормативная база</strong><small>29 пунктов в шести модулях; актуальность редакций требует проверки юристом</small></span><span aria-hidden="true">→</span></a></section><section class="publication-section" id="downloads"><h2>Файлы проекта</h2><p>Исходники, тесты, документы и примеры конфигурации. Без договоров, базы данных, секретов и истории Git. Зависимости и веса моделей не включены.</p><div class="document-actions"><a class="download-primary" href="/docs/downloads/contract-docs-${VERSION}.tar.gz" download>Скачать проект · TAR.GZ</a><a href="/docs/downloads/contract-docs-${VERSION}.tar.gz.sha256" download>Контрольная сумма SHA-256</a></div><p><a href="/docs/downloads/manifest.json">Состав и версия поставки</a></p></section>`));
}
export const includedSource=path=>['package.json','package-lock.json','.gitignore'].includes(path)||/^(server\/|public\/|references\/|plans\/|deploy\/onprem\/|scripts\/|test\/)/.test(path)&&!/^public\/downloads\//.test(path)&&path!=='test/vps-smoke.mjs'||pages.some(p=>p[1]===path);
...
export function buildBundle(){
  ...
  const stage=mkdtempSync(join(tmpdir(),'docs-source-')),name='contract-docs-'+VERSION,folder=join(stage,name),entries=[];
  try {
    mkdirSync(folder);
    ...
    add('README.md',Buffer.from(`# Договоры и риски — исходная поставка ${VERSION}\n\nНачните с [инструкции](references/local-deployment.md).\n\nИсполнитель анализа выбирается переменной DOCS_MODEL_PROVIDER: codex, local или облачный вендор. OCR и импорт нормативных пакетов включены как неподключённые модули. Зависимости, веса, пользовательские данные и секреты не включены.\n`));
    ...
    writeFileSync(join(out,'manifest.json'),JSON.stringify({version:VERSION,sourceCommit:commit,archive:filename,bytes:archive.length,sha256:archiveHash,files:entries},null,2)+'\n');
